...
export class AccountRepository {
  ...
  logIn(credential) {
    return new Promise((resolve, reject) => {
      axios
        ...
        .catch((x) => {
          alert(
            "api fetch content failed with: \n" + x + "\n swicted to local data"
          );
          window.isLocal = true;
          let foundUser = LOCALDATA.account.find(
  ...
  signUp(user) {
    if (!window.isLocal) {
      ...
      return new Promise((resolve, reject) =>
        resolve(LOCALDATA.account.push(user))
      );
    }
  }
  ...
  upadteUserInfo(user) {
    if (!window.isLocal) {
      return new Promise((resolve, reject) => {
        axios
          .patch("placeholder for user data endpoint", user, this.config)
          .then((x) => resolve(x.data))
          .catch((x) => {
            alert(x);
          });
      ...
      return new Promise((resolve, reject) => {
        let foundIndex = LOCALDATA.account.findIndex(
